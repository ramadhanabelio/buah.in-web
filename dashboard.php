...
<body>
	...
	<header class="navbar navbar-expand-white sticky-top bg-white flex-md-nowrap p-0 mb-5 shadow-sm">
		<a class="navbar-brand col-md-auto col-lg-2 me-0 px-3 fs-6 text-center" href="dashboard.php"><img src="img/brand.png" width="30%"></a>
		<form action="" method="post" class="col-md">
			<input type="text" class="form-cari form-control bg-light w-100 rounded-0 border-0" name="keyword" autocomplete="off" id="keyword" placeholder="Pencarian . . ." aria-label=>
		</form>
	...
	<div class="container-fluid mb-4">
		<a href="tambah.php" class="btn btn-green">+ Tambah Data Buah</a>
	</div>
		...
		<table class="table table-striped">
			...
			<?php if (count($hasil_pencarian) > 0) : ?>
				...
				<?php foreach ($hasil_pencarian as $row) : ?>
					<tr>
						<td><?= $i; ?></td>
						<td class="text-center"><img src="uploads/<?= $row["gambar"]; ?>" class="img-responsive" alt="Gambar buah"></td>
						<td><?= $row["nama"]; ?></td>
						<td><?= $row["deskripsi"]; ?></td>
						<td class="aksi">
							<a href="ubah.php?id=<?php echo $row['id']; ?>" class="badge bg-warning text-decoration-none"><i class="bi bi-pencil-square"></i> Ubah</a>
							<button type="button" class="badge bg-danger text-decoration-none border-0" data-bs-toggle="modal" data-bs-target="#hapusModal<?php echo $row['id']; ?>"><i class="bi bi-trash3"></i> Hapus</button>
						</td>
					</tr>
